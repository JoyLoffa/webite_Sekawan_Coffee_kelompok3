...
<form method="POST" action="{{ route('admin.update_produk') }}" enctype="multipart/form-data">
    ...
    <div class="modal-body">
        ...
        <div class="form-group mt-3">
            ...
            <textarea class="form-control @error("deskripsi") is-invalid @enderror" rows="3" name="deskripsi" id="deskripsi" placeholder="">{{$edit->deskripsi}}</textarea>
        </div>
        <div class="form-group mt-3">
        ...
        <div class="form-group mt-3">
            ...
            @error("gambar")
                <span class="text-danger">{{ $message }}</span>
            @enderror
            <a href="{{url ('gambar', $edit->gambar)}}" target="_blank">
                <img src="{{url ('gambar', $edit->gambar)}}" class="img-fluid mt-3" style="width:80px;">
            </a>
        </div>
